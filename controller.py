...
def getPlayers(ligue, cookie):
    r = requests.get('https://api-' + ligue + '.superfantasylol.com/api/v1/playerinfo', headers={
        'cookie': cookie.replace(u"\u2018", "'").replace(u"\u2019", "'"),
        'origin': 'https://' + ligue + '.superfantasylol.com',
        'referer': 'https://' + ligue + '.superfantasylol.com',
        'content-language': 'fr'
...
def getGamesResult(ligue, cookie):
    ...
    with open(ligue + '-matchs.json') as data:
        ...
        for id in matchesId:
            ...
            if result is not None:
                top.append(result.get('local').get('players')[TOP].get('items'))
                top.append(result.get('visitor').get('players')[TOP].get('items'))
    ...
    with open(ligue + '-itemsResults.json', 'w+') as results:
...
def getStats(role, ligue, action):
    ...
    os.chdir('C:/Users/Quentin/PycharmProjects/fantasy')
    with open('./' + ligue + '-itemsResults.json') as results:
        roleResult = json.load(results).get(role)
        # print(roleResult)
    ...
    if action == 'show':
        ...
        os.makedirs('./' + ligue, exist_ok=True)
        pd.DataFrame(object).T.sort_values('pts moy total', ascending=False).to_csv('./' + ligue + '/' + role + '.csv',
                                                                                    index=True)
